...
export function validate(s){
 ...
 for(const v of Object.values(s.flashcards))if(![1,2,3,4].includes(v.box)||!['again','hard','good','easy'].includes(v.lastRating)||!Number.isFinite(v.lastSeen)||!Number.isFinite(v.seen))throw Error('Invalid card progress.');
...
export function rate(id,rating){const old=state.flashcards[id]||{box:1,seen:0};state.flashcards[id]={box:rating==='again'?1:Math.min(4,old.box+({hard:0,good:1,easy:2}[rating]||0)),seen:old.seen+1,lastSeen:Date.now(),lastRating:rating};save();}
