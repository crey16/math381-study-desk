...
export async function learn(){const {app,data,register,sections,options}=await import('./app.js');
 ...
 function render(){const p=pool(),n={new:0,learning:0,known:0};for(const c of p)n[status(c)]++;const pct=p.length?Math.round(100*n.known/p.length):0;
  ...
  else{const correct=picked!==null&&q.opts[picked].id===q.card.id;body=`<p class="muted">Question ${i+1} of ${round.length} · ${esc(title(q.card))} · ${status(q.card)==='learning'?'still learning':status(q.card)}</p><div class="panel"><h2 class="question">${esc(q.card.front)}</h2><div class="choices">${q.opts.map((o,k)=>`<button class="choice ${picked===null?'':o.id===q.card.id?'right':k===picked?'wrong':''}" data-k="${k}" ${picked===null?'':'disabled'}><span class="num">${k+1}</span><div>${markdown(o.back)}</div></button>`).join('')}</div>${picked===null?'<p class="muted">Press 1–4 to choose.</p>':`<p class="${correct?'good':'error'}"><strong>${correct?'Correct.':'Not quite.'}</strong> ${correct?'':'The highlighted answer is right. '}Do you know this card?</p><div class="toolbar"><button id="dont" class="${correct?'':'primary'}">D · Still learning</button><button id="know" class="${correct?'primary':''}">K · I know it</button><small class="muted">Enter picks the highlighted one.</small></div>`}</div>`;}
  app.innerHTML=`<div class="eyebrow">01 / Recall · Learn</div><h1>Learn</h1><div class="toolbar"><a href="#flashcards">Flip cards</a> · <strong>Learn</strong> · <a href="#rapidfire">Rapid-fire →</a></div><div class="toolbar"><label>Section <select id="lnSection">${options(['',...sections],filter.section)}</select></label><label>Priority <select id="lnPriority">${options(['','1','2','3'],filter.priority)}</select></label></div><div class="meter" aria-label="Known cards"><span style="width:${pct}%"></span></div><p class="muted">${n.known} known · ${n.learning} still learning · ${n.new} new · ${pct}% of ${p.length}</p>${body}`;
  for(const [id,key] of [['lnSection','section'],['lnPriority','priority']])document.getElementById(id).onchange=e=>{filter[key]=e.target.value;start();};
 ...
 register(e=>{if(/^[1-4]$/.test(e.key)&&picked===null)pick(+e.key-1);else if(e.key==='k'||e.key==='K')decide(true);else if(e.key==='d'||e.key==='D')decide(false);else if(e.key==='Enter'){e.preventDefault();if(q&&picked!==null)decide(q.opts[picked].id===q.card.id);else if(!q&&round.length)start();}});
 start();
}
