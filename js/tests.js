...
test('Rule-matching items carry four distinct choices including the answer',()=>{for(let k=0;k<60;k++){const it=generators.rulematch();if(it.choices.length!==4||new Set(it.choices).size!==4||!it.choices.includes(it.answer)||it.type!=='choice')throw Error(it.prompt);}return true;});
test('Book form matches Table 1 layout',()=>bookForm(bookRules.mp)==='p\np → q\n---\n∴ q'&&bookForm(bookRules.add)==='p\n---\n∴ p ∨ q');
test('Book mode shows p, q, r only; renamed mode may not',()=>{setRuleForm('book');for(let k=0;k<40;k++){const it=generators.rulematch();if(!/^Which rule is this\?\n/.test(it.prompt)||/[abst]/.test(it.prompt.split('\n').slice(1).join('')))throw Error(it.prompt);}setRuleForm('renamed');return true;});
